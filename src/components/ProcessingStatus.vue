...
const stages = [
  { title: '接收案件文件', description: '確認訴願書與原處分書，建立本次處理流程。', result: '2 份示範文件已備妥', icon: 'upload' },
...
const heading = computed(() => props.complete ? '分析完成，開始審閱案件' : props.running ? current.value.title : '已停止本次模擬分析')
  ...
  <section class="processing-panel" :class="{ complete }" aria-label="案件處理進度">
    <header class="processing-header">
      ...
      <div class="processing-heading" role="status" aria-live="polite" aria-atomic="true">
        <span class="processing-kicker">{{ complete ? '所有階段已完成' : running ? `正在處理 · 第 ${phase + 1} / 6 步` : '流程已暫停' }}</span>
        <h3 ref="headingElement" tabindex="-1">{{ heading }}</h3>
        <p>{{ complete ? 'OCR、程序檢核、法源、草稿與檢核表皆已就緒，請從原始文件開始核對。' : running ? current.description : '選擇的文件仍保留，可以重新開始，不需要再次選檔。' }}</p>
      </div>
    ...
    <footer class="processing-footer"><p><Icon name="info" :size="15" />目前為假資料演示，進度與結果皆為模擬；重新整理會重置。</p><button v-if="running" class="button secondary" @click="$emit('cancel')">停止模擬</button><button v-else-if="complete" class="button primary" @click="$emit('view')">查看 OCR 對照 <Icon name="arrow" :size="16" /></button><button v-else class="button primary" @click="$emit('retry')">重新開始模擬 <Icon name="arrow" :size="16" /></button></footer>
  </section>
</template>
