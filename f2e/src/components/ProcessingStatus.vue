...
const stages = [
  ...
  { title: 'OCR 文字辨識', description: '讀取兩份文件內容，整理案件資訊與訴願主張。', result: 'OCR 對照與案件摘要已回傳', icon: 'scan' },
...
const LIVE_LABELS: Record<string, string> = { header: '表頭', case_type: '案由', appellant: '訴願人', agency: '原處分機關', disposition_ref: '處分', holding: '主文', facts: '事實', reasons: '理由', instruction: '教示', citations: '引用', gaps: '待補查', text: '', source: '來源', section: '', index: '' }
  ...
  <section class="processing-panel" :class="{ complete }" aria-label="案件處理進度">
    <header class="processing-header">
      ...
      <div class="processing-heading" role="status" aria-live="polite" aria-atomic="true">
        ...
        <p>{{ complete ? 'OCR、程序檢核、法源與草稿皆已就緒，請從原始文件開始核對。' : running ? (liveNote || current.description) : error ? error : '選擇的文件仍保留，可以重新開始，不需要再次選檔。' }}</p>
    ...
    <div v-if="running && liveText" class="live-wrap"><div class="live-title"><span class="spinner"></span>模型正在撰寫草稿（即時串流）<span class="live-count">{{ liveText.length }} 字</span></div><pre ref="liveBox" class="live-draft" aria-live="off" aria-label="生成中的草稿">{{ liveTail }}</pre></div>
