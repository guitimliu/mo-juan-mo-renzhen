...
const props = withDefaults(defineProps<{ phase: number; running: boolean; complete: boolean; elapsed: number; files: (string | undefined)[]; error?: string; note?: string }>(), { error: '', note: '請保持此頁開啟，完成後即可核對結果。' })
  ...
  <section class="processing-panel" :class="{ complete }" aria-label="案件處理進度">
    <header class="processing-header">
      ...
      <div class="processing-heading" role="status" aria-live="polite" aria-atomic="true">
        ...
        <p>{{ complete ? 'OCR、程序檢核、法源與草稿皆已就緒，請從原始文件開始核對。' : running ? current.description : error ? error : '選擇的文件仍保留，可以重新開始，不需要再次選檔。' }}</p>
