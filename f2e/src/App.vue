...
const filter = ref('全部')
const source = computed(() => sources.find(s => s.id === selectedSource.value) || sources[0]!)
const filteredSources = computed(() => sources.filter(s => filter.value === '全部' || s.type === filter.value))
...
function stepStatus(index: number) {
  ...
  if (processingVisible.value && !processingComplete.value && !ready.value) return index < progress.value ? '已完成' : index === progress.value ? '已停止' : '等待中'
  if (!ready.value) return index === 0 ? '等待文件' : '等待中'
  return active.value === index ? '目前檢視' : index === 5 ? '待人工確認' : '已完成'
}
function runDemo() {
  if (running.value) return
  if (!useSample.value && files.value.some(f => !f)) return notify('請先選擇訴願書與原處分書兩份圖片')
  running.value = true
  ready.value = false
  reviewed.value = false
  progress.value = 0
