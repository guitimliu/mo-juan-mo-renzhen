...
import { steps, fixture, empty, buildView, stagesFrom } from './data/demo'
...
// envelope → 前端六步進度：0 上傳（POST 成功即完成）、1 OCR＝S1+S2、2 程序＝S2_5、3 檢索＝S3、4 草稿＝S4、5 檢核＝S5
...
async function runDemo() {
  ...
  try {
    await refreshHealth()
    const { case_id } = await createCase(petition, disposition)
    if (myRun !== run) return
    ...
    while (myRun === run) {
      ...
      if (env.status === 'done') {
        ...
        notify(`案件 ${case_id} 六個階段已完成，結果來自後端 API`)
        return
      }
...
function downloadDraft() {
  const content = `訴願決定書草稿【${dataSource.value === 'api' ? modeLabel.value : '本機 fixture'}・非正式決定】\n${caseLabel.value} ${caseTitle.value}\n\n` + draft.value.map(p => p.title + '\n' + p.text + (p.citations.length ? '\n引用來源：' + p.citations.map(c => `${c.label}（${c.id}）`).join('、') : '')).join('\n\n') + '\n\n引用來源\n' + sources.value.map(s => `${s.id}：${s.title}\n${s.content}`).join('\n\n')
  const url = URL.createObjectURL(new Blob(['﻿' + content], { type: 'text/plain;charset=utf-8' }))
  const anchor = document.createElement('a')
  anchor.href = url
  anchor.download = `${caseLabel.value}_訴願決定書_草稿.txt`
  anchor.click()
...
<template>
  <div class="workspace">
    <aside class="sidebar">
      ...
      <div class="side-bottom"><div class="demo-notice"><Icon name="spark" :size="18" /><b>POC 模式</b><p>{{ adapterMode ? `文件只送到本機後端（${modeLabel}）` : '後端未連線，僅能檢視本機 fixture' }}<br>不上雲、不儲存；重新整理即清除</p></div><div class="user"><span class="avatar">E</span><span>前端展示空間<small>新北市 AI 智慧城市黑客松</small></span><span class="online"></span></div></div>
    </aside>

    <div class="main-shell">
      <header class="topbar"><div class="breadcrumb">案件工作台 <Icon name="chevron" :size="13" /><span>{{ caseLabel }}</span></div><div class="topbar-right"><span class="demo-pill" :title="sourceLabel"><span></span>{{ modeLabel }}</span><span class="top-divider"></span><Icon name="scales" :size="18" /><span>新北市政府法制局</span></div></header>
      <main>
        ...
        <div class="section-heading"><div><h2>{{ steps[active]!.title }} <span v-if="active === 4" class="tag">初稿 v1</span></h2><p>{{ steps[active]!.description }}</p></div><button v-if="ready && active === 4" class="button primary" @click="downloadDraft"><Icon name="download" :size="17" />匯出草稿</button><span v-else-if="ready" class="subtle-label">{{ sourceLabel }} · {{ caseLabel }}</span></div>
