...
<div class="foot"><span>單一案例 · 六階段 · 已端到端實測</span><span>01 / 10</span></div>
...
<div class="foot"><span>命題：案件擷取分類 · 法規推薦 · 相似案比對 · 草稿生成</span><span>02 / 10</span></div>
...
<div class="foot"><span>三種主文版本都由 S2.5 規則決定，模型不得自創第四種結論</span><span>03 / 10</span></div>
...
<div class="foot"><span>Docker Compose：Vue 前端（nginx）＋ FastAPI 後端；前端每 1.5 秒輪詢階段狀態</span><span>04 / 10</span></div>
...
<div class="foot"><span>Bedrock 內建 parser 抽不出中文，改自行前處理後再匯入</span><span>05 / 10</span></div>
...
<div class="foot"><span>草稿引用帶 source ID（statutes[0]、precedents[0]…），前端可展開對應原文</span><span>06 / 10</span></div>
...
<div class="foot"><span>未得分的 2 項是正本才有的 LINE 對話內容，輸入文件裡沒有——系統不編造</span><span>07 / 10</span></div>
...
<div class="foot"><span>CloudFront → 彈性 IP → EC2 Docker Compose（Vue＋FastAPI＋PostgreSQL）→ Bedrock Converse（Sonnet 5，降級 4.6）＋ Knowledge Base（Titan v2＋S3 Vectors）｜us-west-2</span><span>08 / 10</span></div>
...
<div class="foot"><span>OCR 頁可見「已去識別化：姓名×3、身分證×1…」標籤；草稿頁顯示還原後真名</span><span>09 / 10</span></div>
...
<div class="foot"><span>github.com/guitimliu/mo-juan-mo-renzhen · 分支 backend</span><span>10 / 10</span></div>
