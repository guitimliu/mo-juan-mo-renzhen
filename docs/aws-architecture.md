...
## 資料流
1. 承辦人瀏覽器（帳密登入）→ CloudFront（HTTPS 子網域 `*.cloudfront.net`，免自備憑證）→ EC2 彈性 IP → nginx（frontend）→ FastAPI（backend）：`POST /api/cases`，WebSocket `/api/cases/{id}/ws` 即時進度。
...
| 服務 | 用途 |
|---|---|
| Amazon CloudFront | 對外 HTTPS 入口（`drod66yo9d2zo.cloudfront.net`），origin 指向 EC2 彈性 IP:80 |
| Elastic IP | EC2 固定對外位址，CloudFront origin 不因重啟改變 |
| Amazon EC2 t3.large（Docker Compose：frontend／backend／db／slides） | 應用主機；SG 只開 22/80/443；`db`＝PostgreSQL 16（案件／影像／草稿版本，volume `pgdata`） |
...
| Amazon S3 | 語料 bucket `mo-juan-mo-renzhen-kb-text`（主辦方 141 篇＋法制局決定書 26,607 篇純文字＋metadata；Block Public Access） |
